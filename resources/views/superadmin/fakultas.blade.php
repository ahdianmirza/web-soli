...
@section('content')
    <main id="main" class="main">
                                    ...
                                    @foreach ($fakultasList as $item)
                                        <tr>
                                            ...
                                            <td style="text-align: center;" class="d-flex gap-2 justify-content-center">
                                                <a href="/fakultasSA/edit/{{ $item->id_fakultas }}" data-bs-toggle="tooltip"
                                                    data-bs-title="Edit" class="btn btn-warning btn-sm"><i
                                                        class="bi bi-pencil-square"></i></a>

                                                <form action="/fakultasSA/delete/{{ $item->id_fakultas }}" method="post"
                                                    id="deleteFakultasForm-{{ $item->id_fakultas }}">
                                                    @csrf
                                                    @method('delete')

                                                    <button onclick="handleDeleteFakultas(event, {{ $item->id_fakultas }})"
        ...
        @include('superadmin.fakultas.fakultas_js')
    </main>
